...
//            Logger.getLogger(KhuyenMaiService_IMPL.class.getName()).log(Level.SEVERE, null, ex);
    ...
    @Override
    public List<KhachHang02> getall02() {
        ...
        try (Connection con = DBConnection.openDbConnection(); PreparedStatement ps = con.prepareStatement(query);) {
            ResultSet rs = ps.executeQuery();
            List<KhachHang02> listSP = new ArrayList<>();
            while (rs.next()) {
                KhachHang02 khachhang = new KhachHang02();
                khachhang.setId(rs.getInt(1));
                khachhang.setTen(rs.getString(2));
                khachhang.setTendem(rs.getString(3));
                khachhang.setHo(rs.getString(4));
...
//               kh.setId(rs.getInt(5));
    ...
    public List<KhachHang02> GetTKTheoIDKH(int ID) {
        List<KhachHang02> listkh = new ArrayList<>();
        try {
            listkh.removeAll(listkh);
...
//            Logger.getLogger(KhuyenMaiService_IMPL.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public List<KhachHang> loadPaging(int pageNum, int fetchRow) {
        ...
        String sql = "SELECT * FROM KHACHHANG ORDER BY id OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
